...
class HomeHeroViewController: UIViewController {
  ...
  func runARSession() {
    // Registers HomeHeroViewController as ARSCNView delegate. You’ll use this later to render objects.
...
extension HomeHeroViewController: ARSCNViewDelegate {
  ...
  func renderer(_ renderer: SCNSceneRenderer, didAdd node: SCNNode, for anchor: ARAnchor) {
    DispatchQueue.main.async {
      if let planeAnchor = anchor as? ARPlaneAnchor {
        ...
        // currentMode is a HomeHeroViewController property already added in the starter. It represents the current UI state: placeObject value if the object button is selected, or measure value if the measuring button is selected. The switch executes different code depending on the UI state.
  ...
  // showMessage(_:label:seconds:) is a helper method included with the starter project. It shows a message in a label for a given number of seconds. session(_:didFailWithError:) is called in case of general ARSession errors like a device not supporting AR.
